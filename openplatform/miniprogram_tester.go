...
func (mp *MiniProgram) BindTester(wechatId string) *http.ResponseData {
	return &http.ResponseData{
		Json: mp.GetClientWithToken().RequestJson("POST", "wxa/bind_tester", g.Map{"wechatid": wechatId}),
	}
}
...
func (mp *MiniProgram) UnbindTester(tester g.Map) *http.ResponseData {
	return &http.ResponseData{
		Json: mp.GetClientWithToken().RequestJson("POST", "wxa/unbind_tester", tester),
	}
}
...
func (mp *MiniProgram) GetTesters() *http.ResponseData {
	return &http.ResponseData{
		Json: mp.GetClientWithToken().RequestJson("POST", "wxa/memberauth", g.Map{"action": "get_experiencer"}),
	}
}
